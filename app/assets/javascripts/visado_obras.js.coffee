class VisadoObrasHelper
  ...
  bind: ()->
    ...
    @area_bruta.on 'blur', (evt) =>
      ...
      @area_bruta.val(@numberToDecimal(@area_bruta_val))
    ...
    $('input').on 'blur', (evt) ->
      ...
      if elem.attr('type') == 'number'
        elem.val(controller.numberToDecimal(elem.val()))

    $('.validate-number').on 'keypress', @alEscribir
  ...
  calcular_costo_construccion_estimado: () =>
    tamano_area_bruta = @area_bruta_val
    tipo_visado = @selector_tipo_visado.val()
    if tamano_area_bruta
      ...
      $('#costo_construccion_estimado').val(BSF+@numberToDecimal(costo_estimado_construccion))
